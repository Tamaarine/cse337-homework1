...
def search(id, desc, priority):
    ...
    filtered_task = dict_task
    
    # Every criteria is None, set filtered_task to be original task
    if id is None and desc is None and priority is None:
        filtered_task = dict_task
    else:
        # At least one criteria is present, use it to do the filtering
        if id is not None:
            filtered_task = {key : val for key, val in filtered_task.items() if key == id}
            
        if desc is not None:
            filtered_task = {key : val for key, val in filtered_task.items() if desc.lower() == val['DESCRIPTION'].lower()}
            
        if priority is not None:
            filtered_task = {key : val for key, val in filtered_task.items() if priority == int(val['PRIORITY'])}

    ret = ""
    for key in filtered_task:
        result = filtered_task[key]
